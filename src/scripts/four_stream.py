...
from dataset.dataset import FourStreamDataset, get_trainval_data_subsets
from models.fusion.lightning_module import FourStreamAttentionLightningModule
...
class FourStreamDataModule(L.LightningDataModule):
    def __init__(
        self,
    ...
    def setup(self, stage: str):
        ...
        (
            ...
            transforms_together,
            transforms_resize,
        ) = FourStreamDataset.get_default_transforms(
            self.loading_mode, self.residual_mode, self.augment, self.image_size
        )

        trainval_dataset = FourStreamDataset(
            trainval_lge_dir,
            trainval_cine_dir,
            trainval_mask_dir,
            trainval_txt_dir,
        ...
        test_dataset = FourStreamDataset(
            test_lge_dir,
            test_cine_dir,
            test_mask_dir,
            test_txt_dir,
        ...
        if self.combine_train_val:
            ...
            valid_dataset = FourStreamDataset(
                trainval_lge_dir,
                trainval_cine_dir,
                trainval_mask_dir,
                trainval_txt_dir,
...
class FourStreamAttentionCLI(I2RInternshipCommonCLI):
...
if __name__ == "__main__":
    file_handler = logging.FileHandler(filename="logs/four_stream.log")
    stdout_handler = logging.StreamHandler(stream=sys.stdout)
    handlers = [file_handler, stdout_handler]
    logging.basicConfig(level=15, format=LOGGING_FORMAT, handlers=handlers)
    logger = logging.getLogger(__name__)

    cli = FourStreamAttentionCLI(
        FourStreamAttentionLightningModule,
        FourStreamDataModule,
        save_config_callback=None,
        auto_configure_optimizers=False,
        parser_kwargs={
            "fit": {"default_config_files": ["./configs/four_stream.yaml"]},
            "predict": {
                "default_config_files": [
                    "./configs/four_stream.yaml",
