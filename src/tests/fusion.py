...
# First party imports
from models.fusion.model import (
    BERTModule,
    FourStreamVisionModule,
    ThreeStreamVisionModule,
)
from models.fusion.segmentation_model import (
...
class TestThreeStreamSegmentationModel:
    batch_size: int = 2
    tokenizer: BertTokenizer = AutoTokenizer.from_pretrained(
        "microsoft/BiomedVLP-CXR-BERT-general", trust_remote_code=True
    )
    ...
    num_frames: int = 10
    caption = "a quick brown fox jumps over the lazy dog."
    res_image = torch.rand(
        (batch_size, num_frames, 1, 224, 224), dtype=torch.float32
    ).cuda()
    image = torch.rand((batch_size, 1, 224, 224), dtype=torch.float32).cuda()

    @torch.no_grad()
    def _test_with_batch(
        self,
        model: ThreeStreamAttentionUnet,
        xs: Tensor,
        xr: Tensor,
        xt: Tensor,
        xta: Tensor,
    ...
    def test_resnet50_smp(self):
        ...
        self._test_with_batch(
            model, self.image, self.res_image, token.cuda(), mask.cuda()
        )
