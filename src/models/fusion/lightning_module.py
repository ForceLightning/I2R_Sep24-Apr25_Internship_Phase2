...
class URRFusionAttentionLightningModule(CommonModelMixin):
    ...
    @override
    def forward(
        self, xs: Tensor, xr: Tensor, xt: Tensor, xta_mask: Tensor
    ...
    def shared_forward_pass(
        ...
        with torch.autocast(device_type=self.device.type, enabled=prefix == "train"):
            ...
            masks_proba, _init_uncertainty, _final_uncertainty, conf_loss = self.model(
                xs_input, xr_input, xt_input, xta_input
            )
    ...
    @override
    def training_step(
        self, batch: tuple[Tensor, Tensor, Tensor, Tensor, Tensor, str], batch_idx: int
    ):
    ...
    def predict_step(
        self,
        batch: tuple[Tensor, Tensor, Tensor, Tensor, Tensor, str | list[str]],
        batch_idx: int,
